...
portal.on('COMPONENT_SEGMENT_LOADED.33c9a987-edff-43ae-8963-1bd3992ea404', function (e) {
  ...
  $('[data-palette]', this).each(function (i, elem) {
    ...
    function render(index) {
      ...
      $item.data({ color: color }).addClass(dark ? 'colors-dark' : 'colors-light');
      $list.append($item);
    ...
    $items.each(function (i, elem) {
      ...
      $color.attr('data-content', ['<p style="color: #bbb;">Copy color code</p>', ClipboardButtonTemplate('HEX', color.toHexString()), ClipboardButtonTemplate('RGB', color.toRgbString())].join(' '));
